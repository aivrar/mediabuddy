...
fn build_new_image(
    ...
    NewImage {
        ...
        file_size: file_size.or(result.file_size),
...
async fn download_photo(
    ...
    let original_bytes: Bytes = resp.bytes().await?;
    let bytes_for_decode = original_bytes.clone();

    let (width, height, thumb_jpeg) = tokio::task::spawn_blocking(
        move || -> Result<(u32, u32, Vec<u8>)> {
    ...
    let (filename, original_rel, file_size) = if preview_only {
        (thumb_filename, String::new(), None)
    } else {
    ...
    let saved = tokio::task::spawn_blocking(move || manager_for_db.add_image(&new))
        .await
        .map_err(|e| AppError::other(e.to_string()))??;
    Ok(Some(saved))
}
...
async fn download_video(
    ...
    let original_rel = if preview_only {
        String::new()
    } else {
...
async fn download_poster(
    ...
    let resp = client
        ...
        .await?;
    if !resp.status().is_success() {
        return Err(AppError::other("poster fetch non-200"));
    }
    let bytes = resp.bytes().await?;
    // Re-encode to a 300px JPEG thumbnail for consistency
    let bytes_for_decode = bytes.clone();
    let (_, _, thumb_jpeg) = tokio::task::spawn_blocking(
        move || -> Result<(u32, u32, Vec<u8>)> {
            let img = image::load_from_memory(&bytes_for_decode)?;
            let thumb = img.thumbnail(THUMB_SIZE, THUMB_SIZE).to_rgb8();
            let (tw, th) = thumb.dimensions();
